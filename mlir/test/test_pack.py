...
def single_function_model(function, inputs, outputs, **kwargs):
    inputs = [ mlir.Value(x[0], shape=x[1], dtype='<f4') for x in inputs ]
    outputs = [ mlir.Value(x[0], shape=x[1], dtype='<f4') for x in outputs ]
    nodes = inputs + outputs
    input_names = [ x.name for x in inputs ]
    output_names = [ x.name for x in outputs ]
    functions = [getattr(mlir.functions, function)(input_names, output_names, **kwargs)]
    actual = mlir.MLIR(function.encode(), b'mlir2chainer_test', "0.1", input_names, output_names, nodes, functions).pack()
    expected = mlir.load(os.path.join(os.path.abspath(os.path.dirname(__file__)), function + '.mlir')).pack()
    print(actual)
    print(expected)
