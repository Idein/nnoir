...
def test_LSTM_02():
    class LSTMTester(Base):
        def create_onnx(self) -> onnx.ModelProto:
            ...
            graph = make_graph([node], "lstm_graph", inputs, outputs, initializer=[W, R, B, seq_lens, init_h, init_c])
            print(onnx.helper.printable_graph(graph))
            model = make_model(graph)
            return model
...
def test_LSTM_03():
    class LSTMTester(Base):
        def create_onnx(self) -> onnx.ModelProto:
            ...
            init_h = from_array(np.ones((1, batch_size, hidden_size)).astype(np.float32), "initial_h")
            init_c = from_array(np.ones((1, batch_size, hidden_size)).astype(np.float32), "initial_c")
            P = from_array(weight_scale * np.ones((1, number_of_peepholes * hidden_size)).astype(np.float32), "P")

            graph = make_graph([node], "lstm_graph", inputs, outputs, initializer=[W, R, B, seq_lens, init_h, init_c, P])
            print(onnx.helper.printable_graph(graph))
            model = make_model(graph)
            return model
