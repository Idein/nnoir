...
def test_resize_00():
    ...
    class ResizeTester(Base):
        def create_onnx(self) -> onnx.ModelProto:
            node = make_node("Resize", inputs=["x", "roi", "scales", "sizes"], outputs=["y"],
            ...
            graph = make_graph([node], "add_graph", inputs, outputs, initializer=[init_roi, init_scales, init_sizes])
...
def test_resize_01():
    ...
    class ResizeTester(Base):
        def create_onnx(self) -> onnx.ModelProto:
            node = make_node("Resize", inputs=["x", "roi", "scales"], outputs=["y"],
                             mode='linear', coordinate_transformation_mode='align_corners')
            inputs = [info("x", TensorProto.FLOAT, (1, 3, 10, 10))]
            ...
            graph = make_graph([node], "add_graph", inputs, outputs, initializer=[init_roi, init_scales])
            return make_model(graph)
