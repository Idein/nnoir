        ...
        for attr in node.attribute:
            if attr.name == "mode":
                if attr.s != b"linear":
                    raise UnsupportedONNXOperation(
                        self.node,
                        f"{attr.s.decode('utf-8')} mode is not supported for Resize, only linear.",
                    )
            if attr.name == "coordinate_transformation_mode":
                if attr.s == b"pytorch_half_pixel":
                    ...
                    raise UnsupportedONNXOperation(
                        self.node,
                        f"{attr.s.decode('utf-8')} coordinate_transformation_mode "
                        "is not supported for Resize, only "
                        '"align_corners" and "pytorch_half_pixel"',
                    )

        if self.mode == b"half_pixel":
            raise UnsupportedONNXOperation(
                self.node,
                'default "half_pixel" coordinate_transformation_mode is unsupported',
            )
