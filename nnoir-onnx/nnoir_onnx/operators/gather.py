...
import numpy as np
import onnx
from nnoir.functions import Function, Slice, Reshape
...
class OpGather(Op):
    ...
    def to_function(self, env: Dict[str, NDArray[Any]], constants: Dict[str, NDArray[Any]]) -> List[Function]:
        ...
        if indices in constants:
            ...
            if indices_v.dtype == np.int64: # TODO: 要調査
                ...
                return [
                    Slice([v0], [inode_name], starts=starts, ends=ends),
                    Reshape([inode_name], list(self.node.output), shape=list(map(int, env[v1].shape)))
                ]
            else:
