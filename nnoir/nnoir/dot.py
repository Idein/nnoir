...
def function_label(function):
    ret = '{{' + ('|'.join([function[b'name'].decode()] + list(map(lambda v: '<a' + str(v[0]) + v[1].decode() + '>', enumerate(function[b'inputs'])))))
...
def to_dot(nnoir, rankdir = 'TB'):
    ...
    for value in values:
        ...
        if value[b'name'] in outputs:
            continue
        attribute = { 'xlabel' : value_name(value),
                      'shape' : 'point' }
        attributes = ["%s=\"%s\"" % (k, v) for (k, v)
                      in attribute.items()]
    ...
    for function in functions:
        # output function
        attribute = { 'label' : function_label(function),
                      'shape': 'record',
                      'style' : 'filled',
        ...
        for i, from_value in enumerate(function[b'inputs']):
            ret += '  %s -> %s:a%d%s[constraint=false];' % (from_value.decode(), function_name(function), i, from_value.decode())
