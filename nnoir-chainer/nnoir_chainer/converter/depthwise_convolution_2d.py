...
class ConvertDepthwiseConvolution2D(chainer.Chain):
    def __init__(self, function, inputs, outputs):
        ...
        self.pad = lambda x: x
        if (
            function.params["pad_h"][0] != function.params["pad_h"][1]
            or function.params["pad_w"][0] != function.params["pad_w"][1]
        ):
            padding = 0
        ...
        with self.init_scope():
            self.conv = L.Convolution2D(
                ...
                initialW=numpy.rollaxis(function.params["W"], 1, 0).reshape(out_channels, in_channels // groups, kh, kw),
