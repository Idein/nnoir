...
class ConvertMaxPooling2D:
    def __init__(self, function, inputs, outputs):
        ...
        def pad(x):
            return F.pad(x, padding, mode="constant", constant_values=-float("inf"))

        self.f = lambda x: F.max_pooling_2d(pad(x), ksize=tuple(function.params["kernel"]), stride=(sy, sx), pad=0)
