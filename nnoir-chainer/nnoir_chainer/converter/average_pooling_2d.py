...
class ConvertAveragePooling2D:
    def __init__(self, function, inputs, outputs):
        ...
        def pad(x):
            return F.pad(x, padding, mode="constant", constant_values=0)
        ...
        if function.params["count_exclude_pad"]:
            ...
            count = F.average_pooling_2d(count, ksize=tuple(function.params["kernel"]), stride=(sy, sx), pad=0)
            self.f = (
                lambda x: F.average_pooling_2d(
                    pad(x),
            ...
            self.f = lambda x: F.average_pooling_2d(pad(x), ksize=tuple(function.params["kernel"]), stride=(sy, sx), pad=0)
