...
def to_nnoir_node(self, inputs, outputs):
    ...
    upward_outputs = [
        nnoir.Value(
            b"v1",
            np.zeros((lstm_out.shape[0], 4 * lstm_out.shape[1])).astype(lstm_out.dtype),
        )
    ]
    ...
    upward_functions_linear_b = (
        encode_ndarray(self.upward.b.data) if (hasattr(self, "b") and self.upward.b is not None) else None
    )
    upward_functions = [
        NNOIR.Linear(
            upward_input_names,
    ...
    lateral_inputs = [
        nnoir.Value(
            b"v0",
            np.zeros((lstm_out.shape[0], lstm_out.shape[1])).astype(lstm_out.dtype),
        )
    ]
    lateral_outputs = [
        nnoir.Value(
    ...
    lateral_functions_linear_b = (
        encode_ndarray(self.lateral.b.data) if (hasattr(self, "b") and self.lateral.b is not None) else None
    )
    lateral_functions = [
        NNOIR.Linear(
            lateral_input_names,
